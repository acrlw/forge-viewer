#ifndef FORGE_COMMON_GLSL
#define FORGE_COMMON_GLSL

const float FORGE_GAMMA = 2.2;
const float FORGE_KNEE = 0.8;
...
vec3 gamma_encode(vec3 c) { return pow(max(c, vec3(0.0)), vec3(1.0 / FORGE_GAMMA)); }

#define FORGE_AMBIENT_GAIN 1.0
vec3 ambient_linear(vec3 a) {
    return srgb_to_linear(clamp(FORGE_AMBIENT_GAIN * a, 0.0, 1.0));
}
...
vec3 tonemap(vec3 c) {
    ...
    if (peak <= FORGE_KNEE) return c;
    float headroom = 1.0 - FORGE_KNEE;
    float mapped = FORGE_KNEE + softroll(peak - FORGE_KNEE, headroom);
