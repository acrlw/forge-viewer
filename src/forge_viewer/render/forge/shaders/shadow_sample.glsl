#ifndef FORGE_SHADOW_SAMPLE_GLSL
#define FORGE_SHADOW_SAMPLE_GLSL

#define SHADOW_MAX_CASCADES 3
#define SHADOW_MAX_LOCAL 8
#define SHADOW_MAX_LIGHTS 16
...
float shadow_factor(vec3 world_pos, vec3 normal, float view_depth) {
    ...
    float tan_theta = sqrt(max(1.0 - ndl * ndl, 0.0)) / max(ndl, FORGE_SHADOW_MIN_NDL);
