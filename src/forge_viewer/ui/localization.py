...
_ZH_CN = {
    ...
    "This Forge scene references model files that are no longer available. Locate files individually or search one directory to repair every unambiguous path.": "此 Forge 场景引用了已不可用的模型文件。可逐个定位文件，或搜索一个目录以修复所有无歧义路径。",
    "Tick scale": "刻度大小",
    ...
    "Save scene": "保存场景",
    "Open Forge scene": "打开 Forge 场景",
    "Add Forge resource directory": "添加 Forge 资源目录",
    "Import mesh": "导入网格",
    ...
    "The current workspace cannot open Forge scene files": "当前工作区无法打开 Forge 场景文件",
    "Unsupported file": "不支持的文件",
    "Select a model or body before creating a site": "创建站点前请先选择模型或本体",
    "New Forge scene": "新建 Forge 场景",
    "Adding model": "正在添加模型",
    ...
    "URDF (*.urdf)": "URDF (*.urdf)",
    "All files": "所有文件",
    "Forge scenes (*.forge.json)": "Forge 场景 (*.forge.json)",
